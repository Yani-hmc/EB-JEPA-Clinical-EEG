...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Spectral loss provides no independent signal (D4 resolved)}:
    a full 4-point sweep ($w_{\rm spec} \in \{0.05, 0.10, 0.20, 0.30\}$) with
    correctly-weighted VICReg (inv=25) yields TUAB BACC $\in [0.795, 0.813]$,
    all within single-seed noise of the no-spectral baseline (0.819).
    Drops of 0.006--0.024\,pp are not statistically significant with single runs.
    The spectral loss was compensating for the buggy invariance underweighting
    (contributing $1 \times 0.023 = 0.023$ vs.\ $25 \times 0.023 = 0.575$ with the fix);
    with correct VICReg the spectral term is redundant.
    The accurate characterisation is \emph{neutral-to-mildly-harmful}, not dramatically
    detrimental --- multi-seed runs would be needed to confirm even a small negative effect.
\end{enumerate}
...
\begin{description}[noitemsep]
  ...
  \item[D4] (\emph{Resolved}) Spectral + fixed VICReg: neutral-to-mildly-harmful.
    Full sweep ($w_{\rm spec} \in \{0.05, 0.10, 0.20, 0.30\}$) shows drops of
    0.006--0.024\,pp TUAB BACC vs.\ baseline, all within single-seed noise.
    Multi-seed confirmation would be needed to rule out a small negative effect entirely,
    but no positive benefit exists at any tested coefficient.
