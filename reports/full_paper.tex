...
A 3-seed ablation (both directions) reveals that VICReg with $\lambda_{\rm inv}{=}1$
(the framework default) achieves $0.819 \pm 0.004$ BACC, while the paper-correct
$\lambda_{\rm inv}{=}25$ achieves only $0.789 \pm 0.005$ --- a $3\,\text{pp}$
deficit at ${\approx}6\sigma$.  This is a deviation from the VICReg vision default,
not a bug: for short-window (10\,s) clinical EEG with a 0.4M conv encoder, the
anti-collapse terms (variance + covariance) should dominate over view-invariance.
Ongoing ablation ($\lambda \in \{1, 5, 10, 25\}$) will clarify the optimal regime.
...
\begin{description}[noitemsep]
  ...
  \item[D7] (\emph{In progress}) Optimal $\lambda_{\rm inv}$ for EEG.
    Jobs capDinv5 and capDinv10 are running ($\lambda \in \{5, 10\}$, alongside the
    known endpoints $\{1, 25\}$).  If the monotone ordering inv=1 $>$ inv=5 $>$ inv=10 $>$
    inv=25 holds, it constitutes the first systematic evidence that EEG SSL requires
    weaker view-invariance than vision SSL.
