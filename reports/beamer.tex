...
\begin{frame}{Latent Space — Do the Representations Cluster?}
\begin{columns}[T]
\column{0.62\textwidth}
\IfFileExists{figures/comparison_3methods.png}{%
  \includegraphics[width=\linewidth]{figures/comparison_3methods.png}
}{%
  \begin{center}
  \textcolor{muted}{\footnotesize [Figure pending — job running on Dalia]}\\
  \vspace{1cm}
  \textcolor{muted}{PCA / UMAP / t-SNE — 276 TUAB eval recordings}
  \end{center}
}
\column{0.36\textwidth}
\textbf{Setup:}
\begin{itemize}
  \item Best encoder: Conv1D + VICReg $\lambda{=}1$ + corruption (BACC 0.819)
  \item 276 TUAB eval recordings
  \item Mean-pool 16 windows $\to$ 256-dim per patient
  \item Project to 2D, color by label (\textcolor{blue!80}{blue} = Normal, \textcolor{red!80}{red} = Abnormal)
\end{itemize}

\vspace{0.5em}
\textbf{Key question:} does a model trained with \emph{no labels} produce representations that are linearly separable?

\vspace{0.3em}
\textcolor{good}{\small BACC 0.819 $\Rightarrow$ yes.}

\vspace{0.3em}
\textcolor{muted}{\footnotesize Labels never used during pretraining — only during the linear probe.}
\end{columns}
\end{frame}
