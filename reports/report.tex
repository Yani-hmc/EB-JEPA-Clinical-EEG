...
\begin{itemize}
  ...
  \item \textbf{Multi-corpus pretraining} (4$\times$ data): frozen 0.812 per-recording,
    fine-tuned 0.837 — identical to TUAB-only fine-tuned (0.837).
...
\begin{itemize}
  ...
  \item \textbf{Our honest position}: per-window 0.775 (below BIOT/LaBraM); per-recording
    0.836 with a simple frozen linear probe, $0.837$ with fine-tuning — competitive with
    supervised EEGNet (per-recording 0.824).
\end{itemize}
