...
resampled to 200\,Hz.  Recordings span 10--120 minutes (clinical monitoring sessions).
...
\centering
\caption{%
  Complete EB-JEPA results on TUAB v3.0.1 (276-recording eval split).
  ``Frozen'' = frozen encoder, MLP probe.
  ``FT'' = entire network fine-tuned during probe training.
  WIN = per-window BACC; REC = per-recording BACC ($N{=}16$ windows, mean-pooled).
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{L{4.0cm}C{1.3cm}C{1.3cm}C{1.3cm}C{1.3cm}L{1.5cm}L{1.4cm}C{1.2cm}}
\toprule
\textbf{Model / Config} &
...
\textbf{Data} \\
\midrule
\multicolumn{8}{l}{\textit{Conv1D encoder (0.4M parameters) -- spectral regularisation ablation}} \\
...
\midrule
\multicolumn{8}{l}{\textit{Conv1D encoder -- corruption augmentation ablation (3 seeds)}} \\
...
\midrule
\multicolumn{8}{l}{\textit{Conv1D encoder -- fine-tuning and multi-corpus}} \\
